#!/usr/bin/env python
"""
Main entrypoint for the leaked Vulnerability Disclosure Mechanism Detector.
"""
...
from leakedcheck import collectors
from leakedcheck.collectors.fallback import add_fallback_mechanisms
from leakedcheck.collectors.github import analyze as analyze_github
from leakedcheck.collectors.ibb import analyze_ibb
from leakedcheck.collectors.librariesio import analyze_librariesio
from leakedcheck.collectors.overrides import check_for_overrides
from leakedcheck.collectors.packagecontent import analyze_packagecontent
from leakedcheck.collectors.tidelift import analyze_tidelift
from leakedcheck.util.context import Context
from leakedcheck.util.normalize import clean_contacts, sanitize_github_url

logger = logging.getLogger('leakedcheck')
logger.setLevel(logging.ERROR)

console = rich.console.Console(highlight=False)
VERSION = importlib.metadata.version("leakedcheck")
...
class LeakedCheck:
    ...
    def execute(self):
        ...
        try:
            importlib.import_module(f"leakedcheck.collectors.{self.purl.type}")
...
def start():
    ...
    parser = argparse.ArgumentParser(prog="KhulnaSoft Vulnerability Disclosure Mechanism Detector")
    ...
    if args.verbose:
        logging.getLogger('leakedcheck').setLevel(logging.DEBUG)

    # Enable an in-memory cache, only if we're running as a script
    requests_cache.install_cache("leakedcheck_cache", backend="memory", allowable_codes=[200, 404])
